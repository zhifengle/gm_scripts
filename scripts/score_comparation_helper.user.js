...
const favicon$2 = 'https://www.google.com/s2/favicons?domain=2dfan.com';
...
function getPageSearchHost(page) {
    if (page.searchApi) {
...
async function runInSiteSearchQueue(page, task) {
...
function getNoMatchInfo(curInfo) {
    return {
        name: curInfo.name,
        url: '',
    };
...
`);
    ...
    const $searchNote = $dialog.querySelector('.e-user-config-note');
    const $status = $dialog.querySelector('.e-user-config-status');
    const $clearCache = $dialog.querySelector('.e-user-clear-cache');
    ...
    const updateSearchSectionState = () => {
        ...
        $dialog
            .querySelectorAll('.e-user-game-page-switch')
            .forEach(($input) => {
    ...
    $dialog.querySelector('.e-user-save-ua').addEventListener('click', () => {
        const $ua = $dialog.querySelector('#e-user-bgm-ua');
