...
const getImageBase64 = require('../utils/getImageBase64.js');
...
;(function() {
  ...
  function addStyle(css) {
    if (css) {
      ...
      GM_addStyle([
        '#e-userjs-amount { padding-left: 10px; border: 0; color: #f6931f; font-size: 20px; font-weight: bold; }',
        '.e-userjs-blur-btn { display: inline-block; text-align: center; width: 60px; height: 30px; line-height: 30px; font-size: 18px; background-color: #f09199; text-decoration: none; color: #fff; margin-left: 50px; margin-bottom: 30px; border-radius: 5px; box-shadow:1px 1px 2px #333; }',
        'canvas:active { cursor: crosshair; }',
        '#e-userjs-blur-preview { display: block; }',
  ...
  function insertBlurInfo($target) {
    const rawHTML = `
    <canvas id="e-userjs-blur-preview" width="8" height="10"></canvas>
    <label for="e-userjs-amount">Blur width and radius:</label>
    <input id="e-userjs-amount" type="text" readonly>
    <br>
    ...
    <br>
    <a href="#" id="reset" class="e-userjs-blur-btn">reset</a>
    <br>
    <input id="e-userjs-blur-url" type="text" placeholder="输入图片链接">
    <a href="#" id="e-userjs-blur-fetch" class="e-userjs-blur-btn">获取图片</a>
  `;
  ...
  function changeInfo($width, $radius) {
    var $info = document.querySelector('#e-userjs-amount');
  ...
  function previewFile($file, $blurInfo) {
    ...
    var $canvas = document.querySelector('#e-userjs-blur-preview')
  ...
  function init() {
    ...
    $blurInfo.addEventListener('change', function(e) {
      if (e.target.type.toLowerCase() === 'range') {
        changeInfo($width, $radius);
      }
    ...
    $blurInfo.addEventListener('click', function(e) {
      if (e.target.id === 'e-userjs-blur-fetch') {
        e.preventDefault()
        var $inputURL = document.querySelector('#e-userjs-blur-url')
        console.log(validURL($inputURL.value));
      }
    ...
    fs.length && fs[0].addEventListener('submit', function (e) {
      ...
      e.preventDefault();
      var dataURL = document.querySelector('#e-userjs-blur-preview').toDataURL('image/png', 1);
