...
async function getAllPageInfo(userId, subjectType, interestType) {
 ...
 */
async function getAllPageInfo$1(userId, subjectType = 'movie', interestType) {
    let res = [];
...
async function getSubjectSearchResults(query, cat = '1002') {
    ...
    loadIframe$1($iframe, url, 1000 * 10);
 ...
 * @returns 搜索结果
 */
async function checkAnimeSubjectExist$1(subjectInfo, type = 'home_search') {
    let query = (subjectInfo.name || '').trim();
    if (!query) {
        console.info('Query string is empty');
...
async function migrateCollection(siteUtils, item, site, type) {
    ...
    if (subjectId) {
        ...
        await siteUtils.updateInterest(subjectId, Object.assign(Object.assign({ interest: typeIdDict$1[type].id }, subjectItem.collectInfo), { rating: subjectItem.collectInfo.score || '' }));
